...
- **Scope**: Azure AD B2C's `/token` endpoint returns a response without an `access_token` field unless the request includes a resource scope, and B2C uses the application's own client ID as that resource scope (see the [Microsoft B2C access-token docs](https://learn.microsoft.com/en-us/azure/active-directory-b2c/access-tokens#openid-connect-scopes); Defra's internal core-service onboarding guidance documents the same convention). Without it, `openid-client` rejects the token-endpoint response with `OAUTH_INVALID_RESPONSE` ("invalid response encountered"). The stub does not enforce this contract. Because the same rejection can also be triggered from the **provider side** — a B2C policy or app-registration change that stops granting the resource scope even though we requested it — the token-endpoint fetch is wrapped (`buildTokenResponseFetch` in `oidc-client.js`) to tolerate a successful response that carries an id_token but no access_token: it patches in an obviously-fake placeholder (nothing in this app uses the access_token) and logs a warning with the granted scope, so a per-environment B2C drift degrades to a log line instead of a login outage.
...
| `OIDC: token response carried no access_token …` (warn)                         | B2C granted no resource scope, so the token response had no `access_token` (the message shows the granted scope). Login proceeds — this app never uses the access_token — but the environment's B2C app registration / policy has stopped granting the client-id scope; raise with Defra ID. Before this tolerance existed, the same condition was a hard login outage: `OIDC callback failed :: … "access_token" property must be a string`. |
